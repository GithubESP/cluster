...
def is_cluster_mod(mod):
    ...
    if ref.startswith("Added Small Passive Skills have"):
        return True
...
def save_config(cfg):
    try:
        ...
        # self.append_log is not available here, need to handle logging differently if needed
    except Exception as e:
        print(f"儲存設定失敗: {e}")
...
class App:
    def __init__(self, root):
        ...
        # 3. 載入詞綴資料 (使用剛剛決定的路徑)
        self.mods = load_mod_list(self.current_mod_file)
    ...
    def _setup_hotkeys(self):
        ...
        try:
            ...
            self.append_log(f"快捷鍵已更新 (改造石: {alt_key}, 星團: {cluster_key}, 開始: {start_key}, 停止: {stop_key})")
        except Exception as e:
    ...
    def create_widgets(self):
        ...
        # --- Prefix Treeview ---
        ttk.Label(right, text="前綴 (雙擊 Min/Max 編輯)").pack(anchor="w")
        prefix_tree_frame = ttk.Frame(right)
        ...
        # --- Suffix Treeview ---
        ttk.Label(right, text="後綴 (雙擊 Min/Max 編輯)").pack(anchor="w", pady=(6,0))
        suffix_tree_frame = ttk.Frame(right)
        ...
        # ---- 快捷鍵設定 ----
        hotkey_frm = ttk.LabelFrame(right, text="快捷鍵設定")
